...
public class MainThreadTask {
        ...
        ReentrantLock lockTunnelB = new ReentrantLock();
            ...
            new Thread(()->{
                do {
                    try {
                        lockTunnelB.lock();
                            if (!tunnelBMain.offer(tunnelA.element(), tunnelB.element().getTrainAcceptableWaitingTime(), TimeUnit.SECONDS)) {
                                System.out.println("");
    ...
    private static BlockingDeque<Train> trainArraysCreatorOfFirstTunnel() throws InterruptedException {
        BlockingDeque tunnel = new LinkedBlockingDeque();
        tunnel.put(new Train(4,3, "Train-1", "Tunnel-1"));
        tunnel.put(new Train(6,10, "Train-2", "Tunnel-1"));
        tunnel.put(new Train(8,3, "Train-3", "Tunnel-1"));
        tunnel.put(new Train(5,3, "Train-4", "Tunnel-1"));
        tunnel.put(new Train(4,3, "Train-5", "Tunnel-1"));
        return tunnel;
    }

    private static BlockingDeque trainArraysCreatorOfSecondTunnel() throws InterruptedException {
        BlockingDeque tunnel = new LinkedBlockingDeque();
        tunnel.put(new Train(4,3, "Train-1", "Tunnel-2"));
        tunnel.put(new Train(6,5, "Train-2", "Tunnel-2"));
        tunnel.put(new Train(8,8, "Train-3", "Tunnel-2"));
        tunnel.put(new Train(5,2, "Train-4", "Tunnel-2"));
        return tunnel;
    }
}
